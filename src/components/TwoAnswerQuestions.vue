<template>
  <section class="quiz">
    <h1 class="text-center text-black font-bold text-2xl mb-5">Two answer question</h1>
      ...
      <div class="flex sm:flex-col sm:items-center items-center justify-center sm:gap-6 gap-4 sm:mb-6">
        <div class=" sm:flex-row flex-col sm:items-end items-center justify-center sm:gap-6 gap-3">
          ...
          <div class="flex items-center mb-4" v-for="(item,index) in question.answers"
               :key="index">
            <input v-model="question.correctAnswer" v-if="question.answers.length > 1 && question.answers[index].option" :id="`checkbox-${index}`" type="checkbox" :value="item.option" name="checkbox" class="w-4 h-4 text-blue-600 bg-gray-100 rounded border-gray-300">
            <label v-if="question.answers.length > 1 && question.answers[index].option" :for="`checkbox-${index}`" class="ml-2 text-sm font-medium text-gray-900 ">{{
...
function addQuiz() {
  let uuid = uuidv4();
  quizStore.addQuiz({...question.value, id: uuid, type: 'two-answer'})
  question.value = {...QUIZ}
